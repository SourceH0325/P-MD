...
import UrlInput from '@/pages/components/docs/edit/EidtUrlInputH'
...
export default function Docs() {
  ...
  useEffect(() => {
    if (id) {
      axios
        ...
        .then((res) => {
          setDocs(res.data.result[0].docs)
          setIsLoading(false)
        })
        .catch((err) => {
          console.log(err)
  ...
  const versionLabel = docs.version

  const editionValue = docs.edition

  const tagVaBel = docs.tag

  useEffect(() => {
    if (tagVaBel) {
      setTag(tagVaBel.map((t) => ({ value: t, label: t })))
    }
    setSelectedVersion({ label: versionLabel })
  ...
  const handleComplete = () => {
    ...
    if (
      ...
      axios
        .put(`/api/docs/editDocsDB/${docs.id}`, doc)
        .then((res) => {
          console.log(res)
          alert('독스 히스토리 편집이 완료되었습니다!')
        ...
        .then((res) => {
          console.log(res)
          router.replace(`/docs/${docs.id}`)
        })
        .catch((err) => {
          console.log(err)
          alert('독스 히스토리 편집에 실패했습니다!')
        })
        .finally(() => {
          setIsSaveButtonDisabled(false)
        })
          ...
          <main className="mb-12">
            <div className="grid gap-4 grid-rows-auto grid-cols-1">
              <div className="grid gap-4 grid-cols-1 mobile:grid-cols-2 grid-rows-auto">
                <NameInput defaultValue={docs.name} />
                <AddressInput defaultValue={docs.address} />
              </div>

              <ExplanationInput defaultValue={docs.explanation} />
              ...
              <div className="grid gap-4 grid-cols-1 mobile:grid-cols-2 grid-rows-auto">
                ...
                <TagInput
                  tag={tag}
                  setTag={setTag}
                  onTagChange={handleChange_tag}
                />
              </div>

              <UrlInput doc={docs} />
              <div className="flex justify-end items-center mt-4 w-auto">
                <button
                  className="font-bold text-lg bg-gray-600 hover:bg-gray-700 px-4 py-2 transition-all mr-2 rounded-lg"
                  onClick={() => router.replace(`/history/${docs.id}`)}
                >
                  취소
                </button>
                <button
