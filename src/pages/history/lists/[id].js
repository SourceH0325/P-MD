    ...
    if (id) {
      ...
      axios
        ...
        .then((res) => {
          ...
          const newLayout = res.data.result[0].list.result_location?.map(
            (location) => ({
              i: location.i,
              x: location.x,
              y: location.y,
          ...
          if (newLayout && newLayout.length > 0) {
            ...
            const newWritingA = res.data.result[0].list.result_content.map(
              (item) => item?.title || '',
            )
            const newWritingB = res.data.result[0].list.result_content.map(
              (item) => item?.content || '',
            )
          ...
          {!isLoading && (
            <>
              ...
              <div className="flex justify-between items-center mt-5">
                ...
                <div
                  id="Complete"
                  className="flex justify-end items-center hidden"
                >
                  <button
                    className="font-bold text-lg bg-gray-600 hover:bg-gray-700 transition-all px-4 py-2 transition-all mr-2 rounded-lg"
                    onClick={() => {
