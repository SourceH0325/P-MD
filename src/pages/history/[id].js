...
export default function Home({ SSDocs }) {
  ...
  const handleSearch = query => {
    ...
    const filteredRemainingHistory = remainingHistory.filter(item => {
      ...
      const isMatch =
        (item.docs &&
          (item.docs.id.toLowerCase().includes(lowerCaseQuery) || item.user.toLowerCase().includes(lowerCaseQuery))) ||
        (item.id &&
          (item.id.toLowerCase().includes(lowerCaseQuery) || item.user.toLowerCase().includes(lowerCaseQuery)));
  ...
  return (
    <>
      <Head>
        <title>{SSDocs && SSDocs.length > 0 ? `${SSDocs[0].name} (히스토리) - 마인독스` : '마인독스'}</title>
        <meta
          name="og:title"
          content={SSDocs && SSDocs.length > 0 ? `${SSDocs[0].name} (히스토리) - 마인독스` : '마인독스'}
        />
                  ...
                  <div key={item._id} className="bg-[#202026] rounded-lg p-5">
                    ...
                    <div className="flex justify-between items-center mt-2">
                      ...
                      <p className="text-lg font-bold text-gray-600 truncate">
                        {item.docs ? '독스 편집' : item.type === 'edit_list' ? '리스트 편집' : '리스트 삭제'}
                      </p>
                    </div>
                    ...
                    <div className="flex justify-start items-center mt-4">
                      <div className="mr-auto">
                        <p
                          ...
                          onClick={() => router.push(item.docs ? `/docs/${item.docs.id}` : `/lists/${item.id}`)}
                        >
                      ...
                      <div className="flex">
                        <button
                          ...
                          onClick={() => router.push(item.docs ? `docs/${item._id}` : `lists/${item._id}`)}
                        >
                          보기
                        </button>
                      </div>
