    ...
    if (id) {
      ...
      axios
        ...
        .then((res) => {
          ...
          const newLayout = res.data.result[0].result_location?.map(
            (location) => ({
              i: location.i,
              x: location.x,
              y: location.y,
          ...
          if (newLayout && newLayout.length > 0) {
            ...
            const newWritingA = res.data.result[0].result_content.map(
              (item) => item?.title || '',
            )
            const newWritingB = res.data.result[0].result_content.map(
              (item) => item?.content || '',
            )
